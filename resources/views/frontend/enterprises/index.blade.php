...
@section('subtitle', trans('cruds.enterprise.title_singular').' '.trans('global.list'))
@section('content')
            ...
            <div class="card">
                <div class="card-header">
                    @can('enterprise_create')
                        <div class="row">
                            <div class="col-lg-12">
                                <a class="btn btn-success" href="{{ route('frontend.enterprises.create') }}">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Enterprise">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($enterprises as $key => $enterprise)
                                    <tr data-entry-id="{{ $enterprise->id }}">
                                        ...
                                        <td>
                                            @can('enterprise_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.enterprises.show', $enterprise->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('enterprise_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.enterprises.edit', $enterprise->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('enterprise_delete')
                                                <form action="{{ route('frontend.enterprises.destroy', $enterprise->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('enterprise_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.enterprises.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-Enterprise:not(.ajaxTable)').DataTable({ buttons: dtButtons })
