...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.enterprise.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.enterprises.update", [$enterprise->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nib">{{ trans('cruds.enterprise.fields.nib') }}</label>
                <input class="form-control {{ $errors->has('nib') ? 'is-invalid' : '' }}" type="text" name="nib" id="nib" value="{{ old('nib', $enterprise->nib) }}" required>
                @if($errors->has('nib'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $enterprise->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('skala_usaha') ? 'is-invalid' : '' }}" name="skala_usaha" id="skala_usaha" required>
                    ...
                    @foreach(App\Models\Enterprise::SKALA_USAHA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('skala_usaha', $enterprise->skala_usaha) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" name="alamat" id="alamat">{{ old('alamat', $enterprise->alamat) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jenis_usaha') ? 'is-invalid' : '' }}" name="jenis_usaha_id" id="jenis_usaha_id">
                    @foreach($jenis_usahas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('jenis_usaha_id') ? old('jenis_usaha_id') : $enterprise->jenis_usaha->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pemilik') ? 'is-invalid' : '' }}" name="pemilik_id" id="pemilik_id" required>
                    @foreach($pemiliks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pemilik_id') ? old('pemilik_id') : $enterprise->pemilik->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $enterprise->description) !!}</textarea>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.enterprises.storeCKEditorImages') }}', true);
...
Dropzone.options.galleryDropzone = {
    url: '{{ route('admin.enterprises.storeMedia') }}',
    maxFilesize: 5, // MB
