@extends('layouts.admin')
@section('content')

<div class="card">
    <div class="card-header">
        {{ trans('global.show') }} {{ trans('cruds.enterpriseDoc.title') }}
    </div>

    <div class="card-body">
        <div class="form-group">
            <div class="form-group">
                <a class="btn btn-default" href="{{ route('admin.enterprise-docs.index') }}">
                    {{ trans('global.back_to_list') }}
                </a>
            </div>
            <table class="table table-bordered table-striped">
                <tbody>
                    <tr>
                        <th>
                            {{ trans('cruds.enterpriseDoc.fields.id') }}
                        </th>
                        <td>
                            {{ $enterpriseDoc->id }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.enterpriseDoc.fields.umkm') }}
                        </th>
                        <td>
                            {{ $enterpriseDoc->umkm->name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.enterpriseDoc.fields.name') }}
                        </th>
                        <td>
                            {{ $enterpriseDoc->name }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.enterpriseDoc.fields.lampiran') }}
                        </th>
                        <td>
                            @foreach($enterpriseDoc->lampiran as $key => $media)
                                <a href="{{ $media->getUrl() }}" target="_blank">
                                    {{ trans('global.view_file') }}
                                </a>
                            @endforeach
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.enterpriseDoc.fields.description') }}
                        </th>
                        <td>
                            {!! $enterpriseDoc->description !!}
                        </td>
                    </tr>
                </tbody>
            </table>
            <div class="form-group">
                <a class="btn btn-default" href="{{ route('admin.enterprise-docs.index') }}">
                    {{ trans('global.back_to_list') }}
                </a>
            </div>
        </div>
    </div>
</div>



@endsection