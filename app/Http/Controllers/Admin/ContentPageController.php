...
class ContentPageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $photo->thumbnail
            ...
            $table->rawColumns(['actions', 'placeholder', 'category', 'tag', 'featured_image', 'lampiran']);
    ...
    public function store(StoreContentPageRequest $request)
    {
        ...
        foreach ($request->input('lampiran', []) as $file) {
            $contentPage->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('lampiran');
        }
    ...
    public function update(UpdateContentPageRequest $request, ContentPage $contentPage)
    {
        ...
        if (count($contentPage->lampiran) > 0) {
            foreach ($contentPage->lampiran as $media) {
                if (!in_array($media->file_name, $request->input('lampiran', []))) {
                    $media->delete();
                }
            }
        }
        $media = $contentPage->lampiran->pluck('file_name')->toArray();
        foreach ($request->input('lampiran', []) as $file) {
